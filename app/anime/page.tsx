...
export default function AnimePage() {
    ...
    useEffect(() => {
        fetch("/api/anime")
            ...
            .catch((err) => {
                console.error("Failed to fetch anime products", err);
                setLoading(false);
            });
    }, []);

    if (loading) return <div className="p-10 text-center">Loading Anime Collection...</div>;

    return (
        <section className="p-6 grid gap-6">
            <h2
                className="text-4xl font-extrabold text-pink-500 uppercase tracking-tighter"
                style={{
