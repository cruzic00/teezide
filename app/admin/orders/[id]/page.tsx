...
export default async function AdminOrderDetail({
  ...
  return (
    <div className="p-8 md:p-10 max-w-4xl">
      <Link href="/admin/orders" className="inline-flex items-center gap-1 text-sm font-medium text-neutral-500 hover:text-neutral-900 mb-6">
        <ChevronLeft size={16} /> Back to orders
      </Link>

      <div className="flex flex-wrap items-end justify-between gap-3 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-neutral-900">Order #{String(order.id).slice(0, 8)}</h1>
          <p className="text-neutral-500 text-sm mt-1">
            {email} · {new Date(order.created_at).toLocaleString()}
          </p>
        </div>
        <p className="text-2xl font-black text-neutral-900">₹{(order.total / 100).toFixed(2)}</p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <Card className="md:col-span-2">
          <div className="p-6 space-y-4">
            <h2 className="font-bold text-neutral-900">Items</h2>
            {(order.items ?? []).map((it: any, i: number) => (
              <div key={i} className="flex items-center gap-4 border-b border-neutral-100 pb-4 last:border-0 last:pb-0">
                ...
                <p className="font-bold text-neutral-900">₹{((it.price * it.qty) / 100).toFixed(2)}</p>
              </div>
            ))}
          </div>
        </Card>
